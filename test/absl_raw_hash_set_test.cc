...
TEST(Group, Match) {
  if (Group::kWidth == 16) {
    ...
    CWISS_ctrl_t group[] = {ctrl_t::kEmpty,    CtrlT(1), CtrlT(2),
...
TEST(Group, CountLeadingEmptyOrDeleted) {
  const std::vector<CWISS_ctrl_t> empty_examples = {ctrl_t::kEmpty,
                                                    ctrl_t::kDeleted};
  ...
  for (CWISS_ctrl_t empty : empty_examples) {
    std::vector<CWISS_ctrl_t> e(Group::kWidth, empty);
    ...
    for (CWISS_ctrl_t full : full_examples) {
      for (size_t i = 0; i != Group::kWidth; ++i) {
        std::vector<CWISS_ctrl_t> f(Group::kWidth, empty);
        f[i] = full;
...
//                 std::is_convertible<const K&,
//                 absl::string_view>::value>::type>
...
//         *alloc, slot, typename slot_type::ctor(),
//         std::forward<Args>(args)...);
//   }
...
//   std::vector<DecomposeType> elem_vector(elem_vector_count,
//   DecomposeType{0}); std::iota(elem_vector.begin(), elem_vector.end(), 0);
...
//     std::list<DecomposeType> elem_list(elem_vector.begin(),
//     elem_vector.end()); expected_copy_constructed =
//     decompose_copy_constructed; DecomposeSet set2(elem_list.begin(),
//     elem_list.end());
...
// This passes but is super slow? I bet my hash function sucks.
TEST(Table, DISABLED_LargeTable) {
  IntTable t;
...
TEST(Table, ClearBug) {
  IntTable t;
  constexpr size_t capacity = Group::kWidth - 1;
...
// // We are injecting non-determinism from the pointer of the table, but do so
// in
// // a way that only the page matters. We have to retry enough times to make
// sure
// // we are touching different memory pages to cause the ordering to change.
// // We also need to keep the old tables around to avoid getting the same
// memory
...
//     if (!found_difference) {
//       FAIL()
//           << "Iteration order remained the same across many attempts with
//           size "
//           << size;
//     }
//   }
...
// // Verify that pointers are invalidated as soon as a second element is
// inserted.
...
//   // Use an assert with side-effects to figure out if they are actually
//   enabled. bool assert_enabled = false; assert([&]() {
//     assert_enabled = true;
//     return true;
//   }());
...
//   IntTable t;
//   // Extra simple "regexp" as regexp support is highly varied across
//   platforms. constexpr char kDeathMsg[] = "Invalid operation on iterator";
